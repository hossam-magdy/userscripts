...
try {
    ...
    var updateElement_dl = function(title, dl_element){
        ...
        $('a', dl_element).first().attr('title', movie_db[title].Title + ' (' +  movie_db[title].Year + ') - ' +  movie_db[title].Plot);
        $('a', dl_element).first().after(' <b style="font-size:80%;">('+movie_db[title].Genre + ')</b>');
        $('a', dl_element).first().on('mouseover', function(e){
            ...
            poster_img.css({
                ...
                top:   scroll_pos + $(window).height()/2 - poster_img.height()/2
            });
